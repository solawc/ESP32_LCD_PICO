...
Error WebCommand::action(char* value, WebUI::AuthenticationLevel auth_level, WebUI::ESPResponseStream* out) {
    ...
    if (_cmdChecker && _cmdChecker()) {
        grbl_send(CLIENT_ALL, "[READ ERR]:1");
...
#    endif
    ...
    static Error mks_frame_run(char* parameter, AuthenticationLevel auth_level) {    // ESP801
        ...
        mks_grbl.is_jump_into_carve = true;

        mks_grbl.is_need_frame = false;
...
#define WEB_COMMON
    ...
    void make_web_settings() {
        ...
        new WebCommand("path", WEBCMD, WU, "ESP801", "SD/Frame", mks_frame_run);
